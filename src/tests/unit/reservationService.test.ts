...
describe('ReservationService', () => {
  ...
  test("initializeTable should get an error 'Tables have already been initialized.'", () => {
    expect(() => reservationService.initializeTable(null)).toThrow('Invalid number of tableCount.');
  });
  ...
  test("initializeTable should get an error 'Tables have already been initialized.'", () => {
    ...
    expect(() => reservationService.initializeTable(10)).toThrow('Tables have already been initialized.');
  });
  ...
  test("reserveTable should get 'Not enough tables available.' error, when Not enough tables left", () => {
    ...
    expect(() => reservationService.reserveTable(41)).toThrow('Not enough tables available.');
  });
  ...
  test("cancelReservation should get an error 'Invalid bookingId.'", () => {
    ...
    expect(reservationService['totalTableCount']).toBe(10);
    expect(reservationService['remainingTable']).toBe(10);
    expect(reservationService['reservationData']).toEqual<Reservation>({} as Reservation);
  });
});
